...
def download_test_images(output_dir: str = "data/test_images", num_images: int = 100):
    ...
    print(f"📥 Downloading test images to {output_path}")

    # Download from LILA dataset
    lila_script = """
    import os
    ...
    script_path = output_path.parent / "download_lila_samples.py"
    with open(script_path, "w") as f:
        f.write(
            lila_script.replace("num_images", str(num_images)).replace(
                "output_dir", f'Path("{output_path}")'
            )
        )
...
def generate_synthetic_images(
    ...
    for i in range(num_images):
        ...
        # Save image
        filename = output_path / f"synthetic_{i:04d}_{width}x{height}.jpg"
...
def plot_benchmark_results(results_file: str, output_dir: str = "results"):
    ...
    summary_data = [
        ...
        [
            "Max FPS",
            f"{df['fps'].max():.2f}",
            f"Batch {df.loc[df['fps'].idxmax(), 'batch_size']}",
        ],
        [
            "Min P95 Latency",
            f"{df['latency_p95'].min():.1f} ms",
            f"Batch {df.loc[df['latency_p95'].idxmin(), 'batch_size']}",
        ],
        [
            "Best Efficiency",
            f"{efficiency.max():.2f}",
            f"Batch {df.loc[efficiency.idxmax(), 'batch_size']}",
        ],
        [
            "Min Memory",
            f"{df['memory_usage_peak'].min():.1f} GB",
            f"Batch {df.loc[df['memory_usage_peak'].idxmin(), 'batch_size']}",
        ],
    ]
    ...
    # Also save as PDF
    pdf_file = output_path / "benchmark_plots.pdf"
...
def compare_with_yolo(md_results: str, yolo_baseline: Dict) -> Dict:
    ...
    comparison = {
        "throughput": {
            ...
            "improvement": (
                (md_df.loc[best_fps_idx, "fps"] / yolo_baseline.get("fps", 1)) - 1
            )
            * 100,
        ...
        "latency": {
            ...
            "improvement": (
                (
                    yolo_baseline.get("latency_p95", 1)
                    / md_df.loc[best_latency_idx, "latency_p95"]
                )
                - 1
            )
            * 100,
        ...
        "memory": {
            ...
            "difference": md_df["memory_usage_peak"].mean()
            - yolo_baseline.get("memory_gb", 0),
        },
        "stability": {
    ...
    print(
        f"  MegaDetector: {comparison['throughput']['md_fps']:.2f} FPS ({comparison['throughput']['md_config']})"
    )
    ...
    print(
        f"  MegaDetector: {comparison['latency']['md_p95']:.1f} ms ({comparison['latency']['md_config']})"
    )
