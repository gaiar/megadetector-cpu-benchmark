...
class MegaDetectorBenchmark:
    ...
    def _configure_cpu_optimization(self):
        ...
        if torch.backends.mkl.is_available():
            print("✓ Intel MKL enabled")
            torch.backends.mkl.enabled = True  # type: ignore[attr-defined]
    ...
    def prepare_test_images(
        self, image_dir: str, num_images: Optional[int] = None
    ) -> List[str]:
    ...
    def benchmark_batch(
        self, image_files: List[str], batch_size: int = 1
    ) -> BenchmarkResult:
            ...
            if batch_images:
                ...
                try:
                    if len(batch_images) == 1:
                        _ = self.detector.generate_detections_one_image(
                            batch_images[0], image_id=batch_ids[0]
                        )
                    else:
                        _ = self.detector.generate_detections_one_batch(
                            batch_images, image_id=batch_ids
                        )
                except Exception as e:
                    print(f"⚠️  Error during inference: {e}")

            batch_time = time.time() - batch_start
            num_processed = len(batch_images) if batch_images else len(batch_files)
            latencies.append(batch_time / num_processed)
        ...
        print(
            f"  • Latency (P50/P95/P99): {result.latency_p50:.1f}/{result.latency_p95:.1f}/{result.latency_p99:.1f} ms"
        )
    ...
    def run_full_benchmark(
        ...
        # Prepare test images
        image_files = self.prepare_test_images(image_dir, num_images=num_images)
        if not image_files:
        ...
        for batch_size in batch_sizes:
            if batch_size > len(image_files):
                print(
                    f"⚠️  Skipping batch_size={batch_size} (only {len(image_files)} images)"
                )
                continue
    ...
    def benchmark_threading(
        self, image_dir: str, thread_counts: Optional[List[int]] = None
    ):
        ...
        for num_threads in thread_counts:
            ...
            self.benchmark_batch(image_files, batch_size=4)
    ...
    def print_summary(self):
        ...
        print("\n📊 Full Results Table:")
        print(
            f"{'Batch':<8} {'FPS':<10} {'P50 (ms)':<10} {'P95 (ms)':<10} {'CPU %':<8} {'RAM (GB)':<10}"
        )
        print("-" * 60)

        for r in sorted(self.results, key=lambda x: x.batch_size):
            print(
                f"{r.batch_size:<8} {r.fps:<10.2f} {r.latency_p50:<10.1f} "
...
def main():
    ...
    parser.add_argument(
        "--model",
        default="/workspace/models/md_v5a.0.0.pt",
        help="Path to MegaDetector model",
    )
    parser.add_argument(
        "--images",
        default="/workspace/data/test_images",
    ...
    parser.add_argument(
        "--mode",
        choices=["full", "threading", "quick"],
        default="full",
        help="Benchmark mode",
    )
    parser.add_argument("--num-images", type=int, help="Limit number of test images")
    ...
    if args.mode == "quick":
        ...
        images = benchmark.prepare_test_images(
            args.images, num_images=args.num_images or 10
        )
        ...
        benchmark.run_full_benchmark(
            args.images, args.batch_sizes, num_images=args.num_images
        )
